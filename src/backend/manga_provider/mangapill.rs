...
static MANGA_PILL_URL: &str = "https://weebcentral.com/";
...
//            Some(cached) => {
//                let doc: String = String::from_utf8(cached.data)?;
//                let response = PopularMangasMangaPill::parse_html(HtmlElement::new(doc))?;
//
...
//        let url = format!(
//
// "{}search/data?{search}&limit={limit}&offset={offset}&sort=Best+Match&order=Descending&official=Any&anime=Any&adult=Any&
// display_mode=Full+Display",            self.base_url
//        );
//
//        let cache = self.cache_provider.get(&url)?;
//
...
//            .await;
//
//        let weebcentral_provider = WeebcentralProvider::new(server.url("/weebcentraltest").parse().unwrap(),
// EmptyCache::new_arc());
//
